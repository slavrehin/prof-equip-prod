...
// Применяем обработку к детальному тексту
if (!empty($arResult['~DETAIL_TEXT'])) {
    $arResult['DETAIL_TEXT'] = wrapImagesInTemplate($arResult['~DETAIL_TEXT']);
}

// Если есть preview text - тоже обрабатываем
if (!empty($arResult['~PREVIEW_TEXT'])) {
    $arResult['PREVIEW_TEXT'] = wrapImagesInTemplate($arResult['~PREVIEW_TEXT']);
}
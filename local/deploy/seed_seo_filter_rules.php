...
$rules = [
    ...
    [
        'CODE' => 'ldogeneratory__proizvoditel',
        'NAME' => 'Льдогенераторы — Производитель',
        'H1' => 'Льдогенераторы #VALUE#',
        'TITLE' => 'Льдогенераторы #VALUE# купить с доставкой в ПРОФЭКВИП',
        'DESCRIPTION' => 'Каталог профессиональных льдогенераторов #VALUE# для баров, ресторанов, кафе и гостиниц. Доставка по всей России и СНГ.',
        'SEO_TEXT' => '<p>ПРОФЭКВИП поставляет профессиональные льдогенераторы #VALUE# для баров, ресторанов, кафе и гостиниц. Лёд нужен для коктейлей, лимонадов и прохладительных напитков, а встроенный бункер позволяет всегда иметь его под рукой в часы наибольшей нагрузки.</p><p>Поможем подобрать льдогенератор #VALUE# под нагрузку вашего заведения: по суточной производительности, размеру кубика, типу охлаждения и условиям установки. Организуем доставку по всей России и СНГ.</p>',
    ],
];

// --only=CODE — обработать только одно правило (не трогать остальные, они могли быть
// отредактированы в админке).
$onlyRule = null;
foreach (array_slice($argv ?? [], 1) as $a) {
    if (strpos($a, '--only=') === 0) { $onlyRule = substr($a, 7); }
}

foreach ($rules as $rule) {
    if ($onlyRule !== null && $rule['CODE'] !== $onlyRule) { continue; }
    $propValues = [
        'H1_TEMPLATE' => $rule['H1'],

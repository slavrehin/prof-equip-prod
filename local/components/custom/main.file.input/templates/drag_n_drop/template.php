...
if ($arParams["ALLOW_UPLOAD"] != "N")
{
    ...
    <div id="file-selectdialog-<?=$uid?>" class="file-selectdialog" style="display:none;max-width: 500px">
...
<script>
    BX.ready(function() {
        ...
        let button = document.querySelector('.write_data');
        BX.addCustomEvent('OnFileUploadSuccess', function(data) {
            console.log(data);

            BX.ajax.runAction('renart:seo.api.Controller.getDataFile', {
                data: {
                    id_file: data.element_id
                }
            }).then(function (response) {
                console.log(response.data);
                button.dataset.item = data.element_id;
                button.style.display='block';
                onSuccessProcessData(response);

            }, function (response) {
            });
        });
        $(document).on('click','.write_data',function () {
            BX.ajax.runAction('renart:seo.api.Controller.addData', {
                data: {
                    id_file: button.dataset.item,
                    class: `<?=$arParams['CLASS']??false?>`
                }
            }).then(function (response) {
                BX('response').innerHTML = response.data;
            }, function (response) {
                BX('response').innerHTML =  response.errors[0].message;
                console.log(response);
            });
        })

    });
    function onSuccessProcessData(response) {
        let data = response.data;
        let titles = Object.keys(response.data[0]);
        console.log(Object.keys(response.data[0]))
        let tableHtml = '<table class="adm-list-table">';
        tableHtml += '<thead><tr class="adm-list-table-header">';
        titles.forEach((item,key) => {
            tableHtml += `<td class="adm-list-table-cell "><div class="adm-list-table-cell-inner">${item}</div></td>`;
        })
        /*tableHtml +=
            '<td class="adm-list-table-cell ">\n' +
            '                <div class="adm-list-table-cell-inner">ID</div></td>' +
            '<td class="adm-list-table-cell ">\n' +
            '                <div class="adm-list-table-cell-inner">URL</div></td>' +
            '<td class="adm-list-table-cell ">\n' +
            '                <div class="adm-list-table-cell-inner">H1</div></td>' +
            '<td class="adm-list-table-cell ">\n' +
            '                <div class="adm-list-table-cell-inner">TITLE</div></td>' +
            '<td class="adm-list-table-cell ">\n' +
            '                <div class="adm-list-table-cell-inner">DESCRIPTION</div></td>';*/
        tableHtml += '</tr></thead><tbody>';

        data.forEach((item,key) => {
            key++;
            tableHtml += '<tr class="adm-list-table-row">';
            titles.forEach((title,key) => {
                tableHtml += `<td  class="adm-list-table-cell align-left">${item[title]}</td>`;
            })
            /*tableHtml +=
                // '<td  class="adm-list-table-cell align-left">'+ key + '</td>' +
                '<td  class="adm-list-table-cell align-left">'+ item['NAME'] + '</td>' +
                '<td  class="adm-list-table-cell align-left">'+ item['H1'] + '</td>' +
                '<td  class="adm-list-table-cell align-left">'+ item['TITLE'] + '</td>' +
                '<td  class="adm-list-table-cell align-left">'+ item['DESCRIPTION'] + '</td>'*/
            tableHtml += '</tr>';
        });

        tableHtml += '</tbody></table>';
        BX('result_file').innerHTML = tableHtml;
    }
</script>

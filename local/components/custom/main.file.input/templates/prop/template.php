...
<tr class="file-inline-file" id="wd-doc#element_id#">
	<td class="files-name">
	<img style="max-height: 70px;" src="#img_src#" alt="">
...
<a href="javascript:void(0);" id="file-selectdialogswitcher-<?=$uid?>" class="file-selectdialog-switcher" <?
	?>onclick="BX.onCustomEvent(this.parentNode, 'BFileDLoadFormController');return false;"><span><?
		?><?=($arParams["ALLOW_UPLOAD"] == "I" ? "Загрузить картинки" : "Загрузить файлы")?></span></a>
<div id="file-selectdialog-<?=$uid?>" class="file-selectdialog" style="display:none;">
	...
	<div class="file-extended">
		...
		<div class="file-placeholder">
			<table class="files-list" cellspacing="0">
				...
				<?if (is_array($arValue) && sizeof($arValue) > 0)
				{
					...
					{
						?><?=str_replace(
							array("#img_src#","#element_id#", "#name#", "#size#"),
							array(CFile::GetPath($arElement['ID']),intval($arElement['ID']), htmlspecialcharsEx($arElement['ORIGINAL_NAME']), CFile::FormatSize($arElement["FILE_SIZE"])),
							$thumb
						);
					}
				}?>
		...
		<div class="file-selector" style="margin: 10px;    padding: 5px 3px 30px 173px;">
            Перетащите файл в эту область<br />
			<span class="file-uploader">
                <span class="file-but-text">или выберите файл на компьютере</span>
                <input class="file-fileUploader <?=$addClass?>" id="file-fileUploader-<?=$uid?>"
                       type="file" <?=$arParams["MULTIPLE"] === 'Y' ? ' multiple="multiple"' : ''?> size='1' /></span>
